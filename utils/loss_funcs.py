...
from utils.SMPL import batch_rodrigues
from utils.geometry import projection_torch, batch_compute_similarity_transform_torch
...
def compute_losses(out, target, mse_func, device, args):
    ...
    if args.theta_loss > 0:
        B, N, _ = target['theta'].size()
        # 72-> 24x 3x3 rotmats
        target_rotmats = batch_rodrigues(target['theta'].view(-1, 3)).view(B, N, 24, 3, 3)

        if args.use_geodesic_loss:
            eps = 1e-6
            # square geodesic loss arccos[(Tr(R1R2^T) -1 )/2]
            trace_rrt = torch.sum(out['pred_rotmats'] * target_rotmats, dim=(-2, -1))  # [B, N, 24]
    ...
    if args.beta_loss > 0:
        # print('beta', out['beta'].shape, target['beta'].shape)
...
#     verts_2d = projection_torch(verts, cam_intr)  # [B, T, 6890, 2]
...
#     pred_flows = pred_flows[:,1:,:,:,:]
#     _flows = pred_flows.contiguous().view(-1, 2, H, W)  # [BT, 2, H, W]
#     sampled_flow = F.grid_sample(_flows, flow_indices)  # [BT, 2, H, W]  (u, v)
#     sampled_flow = sampled_flow.permute(0, 2, 3, 1).view(B, T-1, H * W, 2)[:, :, :6890, :].clone()
#
