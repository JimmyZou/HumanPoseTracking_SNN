...
class H36MDataloader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for t in range(self.num_steps + 1):
            ...
            if do_flip:
                pose_t = np.load(
                    "{}/h36m_events_processed/{}/poses_flip/pose_{:04d}.npz".format(
                        self.data_dir, action, idx
                    )
                )
            else:
                pose_t = np.load(
                    "{}/h36m_events_processed/{}/poses/pose_{:04d}.npz".format(
                        self.data_dir, action, idx
                    )
                )
            ...
            theta.append(pose_t["body_pose"][np.newaxis])  # [1, 72]
            beta.append(pose_t["beta"][np.newaxis])  # [1, 10]
            ...
            if self.modality == "rgb" or self.modality == "all":
                ...
                img_name = "{}/h36m_rgb_processed/{}/imgs/color_{:04d}.jpg".format(
                    self.data_dir, action, idx
                )
            ...
            if self.modality == "events" or self.modality == "all":
                ...
                events_frames = []
                for i in range(gap):
                    frame_name = "{}/h36m_events_processed/{}/events_frame_{:02d}/events_{:04d}.npz".format(
        ...
        if self.modality == "rgb" or self.modality == "all":
            imgs = np.stack(imgs, axis=0).astype(np.float32) / 255.0  # [T, H, W, C]
        ...
        if self.aug:
            ...
            if self.modality == "rgb" or self.modality == "all":
                ...
                imgs_aug = np.transpose(
                    imgs_aug.reshape([self.img_size, self.img_size, self.num_steps, 3]),
                    [2, 0, 1, 3],
                )
    ...
    def obtain_all_clips(self):
        save_filename = "{}/h36m_{}_{:02d}{:02d}.pkl".format(
        ...
        if os.path.exists(save_filename):
            ...
            all_clips = []
            tmp = sorted(os.listdir("{}/h36m_events_processed".format(self.data_dir)))
            action_names = []
            for action in tmp:
                # TODO select camera
                subject = action[0:4]
                if self.mode == "test":
                    if subject in ["s_09", "s_11"]:
                        action_names.append(action)
                    gap = 2  # frame gap, evaluate every 64 frames
                    # will select motion at [0, 8, 16, 24, ..., 64, ...] fps=50
                    skip = self.num_steps * gap  # clip gap
                else:
                    if subject not in ["s_09", "s_11"]:
                        action_names.append(action)
                    gap = self.max_gap  # frame gap
                    skip = self.num_steps // 2  # clip gap
            ...
            for action in action_names:
                all_pose_files = list(
                    sorted(
                        os.listdir(
                            "{}/h36m_events_processed/{}/poses".format(
                ...
                for idx in range(0, n - self.num_steps * gap - 1, skip):
                    ...
                    if end_frame_idx == end_pose_idx:
                        # action, frame_idx
                        all_clips.append(("h36m", action, frame_idx, count))
                        count += 1

            joblib.dump(all_clips, save_filename, compress=3)
            print("save as {}".format(save_filename))

        print("[h36m {}] {} samples".format(self.mode, len(all_clips)))
        return all_clips
    ...
    def visualize(self, idx, save_dir):
        ...
        demo_name = "{}/h36m_{}_{:04d}.gif".format(save_dir, action, frame_idx)
        imageio.mimsave(demo_name, frames, "GIF", duration=gap * 1.0 / 10.0)
        print("save as {}".format(demo_name))


if __name__ == "__main__":
    os.environ["OMP_NUM_THREADS"] = "1"
    # data_train = H36MDataloader(
    #     data_dir="/data/shihao",
    #     smpl_dir="../../smpl_model/models/smpl/SMPL_MALE.pkl",
    #     mode="train",
    #     num_steps=8,
    ...
    #     augmentation=True,
    # )
    # data_train.visualize(4000, "../../examples")

    data_test = H36MDataloader(
