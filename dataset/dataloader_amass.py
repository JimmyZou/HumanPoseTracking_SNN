...
class AMASSDataloader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # dataset_name, action, frame_idx, gap = 'amass', 'MPI_HDM05/tr/HDM_tr_05-03_04_120_poses', 704, 2
        ...
        if self.aug:
            ...
            if not os.path.exists(
                "{}/amass_events/{}/poses_flip".format(self.data_dir, action)
            ):
                do_flip = False
        ...
        for t in range(self.num_steps + 1):
            ...
            if do_flip:
                pose_t = np.load(
                    "{}/amass_events/{}/poses_flip/pose_{:04d}.npz".format(
                        self.data_dir, action, idx
                    )
                )
            else:
                pose_t = np.load(
                    "{}/amass_events/{}/poses/pose_{:04d}.npz".format(
                        self.data_dir, action, idx
                    )
                )
            ...
            if self.modality == "rgb" or self.modality == "all":
                img_name = "{}/amass_rgb/{}/imgs/{:04d}.jpg".format(
                    self.data_dir, action, idx
                )
            ...
            if self.modality == "events" or self.modality == "all":
                ...
                events_frames = []
                for i in range(gap):
                    frame_name = "{}/amass_events/{}/events_frame_{:02d}/events_{:04d}.npz".format(
    ...
    def obtain_all_clips(self):
        save_filename = "{}/amass_{}_{:02d}{:02d}.pkl".format(
        ...
        if os.path.exists(save_filename):
            ...
            action_names = []
            for set_name in os.listdir("{}/amass_events".format(self.data_dir)):
                for subset_name in os.listdir(
                    "{}/amass_events/{}".format(self.data_dir, set_name)
                ):
                    files = os.listdir(
                        "{}/amass_events/{}/{}".format(
                            self.data_dir, set_name, subset_name
                        )
                    )
                    for file in files:
                        pose_filename = "{}/{}/{}".format(
                            set_name, subset_name, file
                        )  # no .npz
                        action_names.append(pose_filename)
            ...
            for action in action_names:
                all_pose_files = list(
                    sorted(
                        os.listdir(
                            "{}/amass_events/{}/poses".format(self.data_dir, action)
                        )
                    )
                )
                n = len(all_pose_files)
                for idx in range(0, n - self.num_steps * self.max_gap - 1, skip):
                    pose_file = all_pose_files[idx]

                    frame_idx = int(pose_file.split("_")[1].split(".")[0])
                    end_frame_idx = frame_idx + self.num_steps * self.max_gap + 1
                    end_pose_idx = int(
                        all_pose_files[idx + self.num_steps * self.max_gap + 1]
                        .split("_")[1]
                        .split(".")[0]
                    )
                    if end_frame_idx == end_pose_idx:
                        # action, frame_idx
                        all_clips.append(("amass", action, frame_idx, count))
                        count += 1

            joblib.dump(all_clips, save_filename, compress=3)
            print("save as {}".format(save_filename))

        print("[amass {}] {} samples".format(self.mode, len(all_clips)))
        return all_clips
    ...
    def visualize(self, idx, save_dir):
        ...
        demo_name = "{}/amass_{}_{:04d}.gif".format(
            save_dir, "-".join(action.split("/")), frame_idx
        )
